...
const corsHeaders = {
  "Access-Control-Allow-Origin": "https://leadsnearby.in",
...
serve(async (req) => {
  ...
  try {
    ...
    // Optional: raw_message might be empty (e.g. image only)
    const messageContent = raw_message || "";

    // if (!raw_message) { ... }  <-- Removed strict check
    ...
    if (override_data) {
      ...
      if (!customerPhone) {
        // Fallback to sender phone if extraction failed
        const fallbackPhone = sender_phone?.replace(/^91/, "").slice(-10);
        if (!fallbackPhone) {
          return new Response(
            JSON.stringify({
      ...
      // Validate address - RELAXED
      if (!parsed.location_address || parsed.location_address.length < 3) {
        // Instead of failing, allow it with a default value
        parsed.location_address = "Unknown Location";
        finalConfidence = Math.max(finalConfidence - 20, 10); // Lower confidence
      }

      finalData = {
        customer_name: parsed.customer_name,
        customer_phone: customerPhone || sender_phone?.replace(/^91/, "").slice(-10),
