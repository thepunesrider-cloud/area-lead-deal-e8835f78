...
import { 
  ...
  Camera, Loader2, MessageSquare, Upload
} from 'lucide-react';
...
const LeadDetails: React.FC = () => {
  ...
  const handleRejectLead = async () => {
    ...
    if (error) {
      ...
      toast({
        title: t('success'),
        description: 'Lead released back to available',
      });
  ...
  const handleCompleteLead = async () => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: t('error'),
        description: 'Failed to complete lead. Make sure proof is uploaded.',
      });
    } finally {
      setActionLoading(false);
    }
  ...
  const openWhatsAppChat = (phone: string, isGenerator: boolean) => {
    const message = encodeURIComponent(
      `Hi, I'm contacting you about lead #${lead?.id?.slice(0, 8)} for ${t(serviceTypeLabels[lead?.service_type || 'other'])}.`
    );
